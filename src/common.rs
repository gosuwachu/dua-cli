...
impl Metadata for FSMetadata {
    ...
    #[cfg(windows)]
    fn size_on_disk(parent: &Path, name: &Path) -> io::Result<u64> {
...
pub trait Walker {
    fn into_iter(&self, path: &Path, root_device_id: u64, options: WalkOptions) -> Box<dyn Iterator<Item = Result<Box<dyn WalkerDir>, jwalk::Error>>>;
}
...
impl Iterator for JWalkWalkerIterator {
    type Item = Result<Box<dyn WalkerDir>, jwalk::Error>;
